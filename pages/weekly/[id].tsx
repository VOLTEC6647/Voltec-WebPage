...
import { fetchNewspaperIssue } from "../../lib/wordpress/newspaper";
...
const Newspaper: NextPage<Props> = ({ newspaper }) => {
  return (
    <div>
      {newspaper && (
        <>
          ...
          <div className="content max-w-4xl mx-auto h-full px-4 pb-16">
            <div className="information pt-8">
              ...
              <div
                className="text-neutral-600 font-manrope tracking-tighter text-lg"
                dangerouslySetInnerHTML={{
                  __html: newspaper.acm_fields.description,
                }}
              />
            </div>
...
export async function getServerSideProps(context: NextPageContext) {
  ...
  try {
    const url = `https://${process.env.NEXT_PUBLIC_WORDPRESS_HOSTNAME}/wp-json/wp/v2/newspapers/${id}?_embed`;
    const res = await fetch(url);
    const newspaper = await res.json();
